...
with open("./output/result_gpt4o_evidence_summary.json", 'r', encoding='utf-8') as file:
...
for index, (external, inspection, relevant, filter_idx, summary) in enumerate(
    ...
    if len(evidence) != 0:
        ...
        selected_items = random.sample(text_list, 1 if len(text_list) > 1 else len(text_list))
        final_text = '\n\n'.join(selected_items)
    ...
    filename = "./output/result_gpt4o_evidence_summary.json"
